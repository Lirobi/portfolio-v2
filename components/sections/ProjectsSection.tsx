...
const iconMap: Record<string, LucideIcon> = {
  ShoppingCart, Flower2, BarChart3, Camera, Calendar, Globe, Smartphone, Image, Code2,
};
...
export default function ProjectsSection({ projects }: Props) {
  ...
  const items = projects.map((p) => ({ ...p, iconComponent: iconMap[p.icon] || Code2 }));
    ...
    <section ref={ref} id="projets" className="py-32 px-6 md:px-12 lg:px-24 2xl:px-12 relative">
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        ...
        >
          ...
          <h3 className="text-2xl font-bold mb-2">{config.content.projectTeaserTitle}</h3>
          <p className="text-foreground/60">{config.content.projectTeaserDescription}</p>
        </motion.div>
      </div>
    </section>
  );
